...
function GraficoEspessura({
  ...
  const [limites, setLimites] = useState({ vermelho: 29, amarelo: 23, verde: 0 }); 
  ...
  useEffect(() => {
    const docRef = doc(db, 'limites', 'espessura');

    const unsubscribe = onSnapshot(docRef, (snapshot) => {
      if (snapshot.exists()) {
        setLimites(snapshot.data());
      } else {
        console.warn("Documento 'espessura' não encontrado em 'limites'.");
      }
    }, (error) => {
      ...
      {!fullscreen && (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h2 className="subheader">Espessura</h2>
            <button
        ...
        <div style={{ width: '100%', height: fullscreen ? '100%' : 440 }}>
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <YAxis domain={[limites.amarelo - 0.2, limites.vermelho + 0.2]} stroke="#ccc" />
...
export default GraficoEspessura;
