...
function GraficoSecagem({
  ...
  useEffect(() => {
    const docRef = doc(db, 'limites', 'secagem');

    const unsubscribe = onSnapshot(docRef, (snapshot) => {
      if (snapshot.exists()) {
        setLimites(snapshot.data());
      } else {
        console.warn("Documento 'secagem' não encontrado em 'limites'.");
      }
    }, (error) => {
      ...
      {!fullscreen && (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h2 className="subheader">Secagem</h2>
            <button
