...
function Densidade() {
  ...
  const obterMedicoesPorData = async (dataStr) => {
    const q = query(collection(db, "medicoes_densidade"), where("data", "==", dataStr));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const q = query(
        collection(db, "medicoes_densidade"),
        where("turno", "==", turno),
        where("data", "==", data),
        where("tipo", "==", tipo),
        where("celula", "==", celula),
      ...
      await addDoc(collection(db, "medicoes_densidade"), {
        ...form,
        valor: Number(valor),
  ...
  return (
    <div className="container">
      <h1 className="header">Registrar Medição Densidade</h1>
